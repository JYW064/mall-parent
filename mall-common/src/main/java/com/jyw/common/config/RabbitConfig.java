...
public class RabbitConfig {
    ...
    DirectExchange orderDirect(){
        return new DirectExchange("xmall.order.direct.exchange",true,false );
    }
    /**
     * 订单延迟队列交换机
     */
    ...
    DirectExchange orderDLDirect() {
        return new DirectExchange("xmall.order.cancel.dlExchange",true,false);
    }
    /**
     * 死信队列
     * @return
    ...
    public Queue orderDLQueue(){
        return new Queue("xmall.order.cancel.queue", true, false, false);
    }
    /**
     * 普通队列
     */
    ...
    public Queue orderQueue() {
        return QueueBuilder
                .durable("xmall.order.direct.queue")
                .withArgument("x-dead-letter-exchange", "xmall.order.cancel.dlExchange")//到期后转发的交换机
                .withArgument("x-dead-letter-routing-key", "xmall.order.cancel")//到期后转发的路由键
                .build();
    }
    /**
     * 绑定普通交换机和队列
